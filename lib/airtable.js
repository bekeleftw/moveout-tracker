...
export async function getCompanyBySlug(slug) {
  const records = await getBase()("Companies")
    .select({
...
export async function getPropertiesForCompany(slug) {
  const records = await getBase()("Properties")
    .select({
...
export async function getUtilityTransfersForProperties(propertyIds) {
  ...
  const records = await getBase()("Utility Transfers")
    .select({
...
export async function updateUtilityTransfer(recordId, fields) {
  ...
  const record = await getBase()("Utility Transfers").update(recordId, clean);
  return {
    id: record.id,
